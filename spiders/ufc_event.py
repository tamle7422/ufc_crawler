...
from ..hf_ufc import setDate,setLocationFightStats,setFightId,setFighterName,setFighterResult,setFighterResultEncode, \
    ...
    setLegSigStrikes,setDistanceSigStrikes,setClinchSigStrikes,setGroundSigStrikes,setSignificantStrikesTable,setUrlEvent, \
    setLocationEvent,setDateEvent,loadFightStatsItem,loadEventItem,setDateEventDetails,setLocationEventDetails, \
    setFighterNameEvent,setFighterResultEvent, \
    setKnockdownEvent,setStrikesEvent,setTakedownEvent,setSubmissionEvent,setWeightClassEvent,setFightDetailsEvent, \
    setFightLastRoundEvent,setFightTimeEvent,loadEventDetailsItem, \
    checkEmpty

class UfcEventScraper(scrapy.Spider):
    name = "ufc_event"
    ...
    custom_settings = {
        'ITEM_PIPELINES': {
            'ufc_crawler.pipelines.UfcEventPipeline': 382,
        },
        "CLOSESPIDER_ITEMCOUNT": 5499
    }

    configure_logging(install_root_handler=False)
    logging.basicConfig(filename='ufc_event_log.txt',format='%(levelname)s: %(message)s',level=logging.INFO,
        filemode="w+")

    def __init__(self,*args,**kwargs):
        super(UfcEventScraper,self).__init__(*args, **kwargs)
        self.urlEventList = ""
        self.dateEventList = ""
        self.locationEventList = ""
        self.dateEventDetails = ""
        self.locationEventDetails = ""
        self.f1Name = ""
        self.f2Name = ""
        self.f1Result = ""
        self.f2Result = ""
    ...
    def parse(self,response):
        try:
            ...
            for i in range(0,len(self.urlEventList)):
                loader = loadEventItem(self,response,i)
                yield loader.load_item()

                yield SplashRequest(url=self.urlEventList[i], callback=self.parseEventDetails, endpoint="execute",
    ...
    def parseEventDetails(self,response,**kwargs):
        try:
            ...
            setDateEventDetails(self,**kwargs)
            setLocationEventDetails(self,**kwargs)

            for i in range(0,len(trTags)):
                # print("event details => {0}".format(countEventDetails))
                # countEventDetails += 1
                setFighterNameEvent(self,trTags,i)
                setFighterResultEvent(self)
                setKnockdownEvent(self,trTags,i)
                setStrikesEvent(self,trTags,i)
                setTakedownEvent(self,trTags,i)
                setSubmissionEvent(self,trTags,i)
                setWeightClassEvent(self,trTags,i)
                setFightDetailsEvent(self,trTags,i)
                setFightLastRoundEvent(self,trTags,i)
                setFightTimeEvent(self,trTags,i)
                loader = loadEventDetailsItem(self,response)
                yield loader.load_item()

        except Exception as ex:
            print("exception --- error in parse event details => {0}".format(ex))
            logging.info("exception --- error in parse event details => {0}".format(ex))
